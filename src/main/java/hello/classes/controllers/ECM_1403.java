...
import hello.classes.bodies.ECM_1403_bodies;
...
//@RestController
public class ECM_1403 {

    private ECM_1403_bodies ecm_1403_bodies = new ECM_1403_bodies();
...
//        printLogs("./1386_SMEAccountRequestSBOS_REST_logs/logs.txt", requestLog.toString());
        ...
        String responseBody = ecm_1403_bodies.getBodyFiles(uuid);
        try {
        ...
        //пауза
//        pause(2); //Из требований к производительности, т.к. НТ не завершено
        pause(4); //x1.5
...
//        printLogs("./1386_SMEAccountRequestSBOS_REST_logs/logs.txt", responseLog.toString());
        ...
        stringToInfluxDB.setMethod("POST_/files");
    ...
    @PostMapping("/api/v1/signature/ecm/sign")
    public String ecmSign(HttpServletRequest request, HttpServletResponse response,
                          @RequestHeader Map<String, String> headers, @RequestBody(required = false) String body){
...
//        printLogs("./1386_SMEAccountRequestSBOS_REST_logs/logs.txt", requestLog.toString());
        ...
        //Добавляем тело для ответа
        String responseBody = ecm_1403_bodies.getBodyEcmSign();
        try {
            response.getWriter().write(responseBody);
        } catch (IOException e) {
            e.printStackTrace();
...
//        printLogs("./1386_SMEAccountRequestSBOS_REST_logs/logs.txt", responseLog.toString());
        ...
        //Записываем в файл для дальнейшей отправки в InfluxDB
        StringToInfluxDB stringToInfluxDB = new StringToInfluxDB();
        stringToInfluxDB.setMethod("POST_/api/v1/signature/ecm/sign");
        stringToInfluxDB.setTimestamp(String.valueOf(System.currentTimeMillis()));
        printLogs("./1386_AccR_REST_logs/logs.txt", stringToInfluxDB.toString());
...
//        printLogs("./1386_SMEAccountRequestSBOS_REST_logs/logs.txt", requestLog.toString());
        ...
        //Устанавливаем статус ответа 204
...
//        printLogs("./1386_SMEAccountRequestSBOS_REST_logs/logs.txt", responseLog.toString());
        ...
        //Записываем в файл для дальнейшей отправки в InfluxDB
        StringToInfluxDB stringToInfluxDB = new StringToInfluxDB();
        stringToInfluxDB.setMethod("DELETE_/files/{file_guid}");
        stringToInfluxDB.setTimestamp(String.valueOf(System.currentTimeMillis()));
        printLogs("./1386_AccR_REST_logs/logs.txt", stringToInfluxDB.toString());
